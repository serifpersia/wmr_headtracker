...
def setup_device():
    ...
    hmd.open_path(path)
    print("\n--- Waking up device ---")
    print("1. Sending Standard Wake...")
    hmd.write(bytes([0x00, 0x02, 0x07]))
    time.sleep(0.05)
    try:
        print("2. Sending Padded Wake (alternative)...")
        hmd.write(bytes([0x02, 0x07] + [0] * 62))
    except OSError as e:
        print(f"   (Note: Alternative wake failed. This is expected on some systems: {e})")
    print("3. Device is awake, locking on IMU stream...")
    return hmd
...
def calibrate_imu(hmd):
    ...
    while samples < target_samples:
        ...
        pkt_avg = np.mean(raw_gyro, axis=1) # [c0, c1, c2]
        gyro_sum += pkt_avg
        samples += 1
...
def process_packet(data_bytes, fusion_obj, last_ts):
    ...
    for i in range(4):
        ...
        diagnostics.append(
            {
                "udp_packet": (eulers[0], -eulers[1], eulers[2]),
            }
        )
...
def run_tracker():
    ...
    try:
        ...
        while True:
            ...
            diagnostics, last_gyro_ts = process_packet(
                bytearray(data), fusion, last_gyro_ts
            )
            if not diagnostics:
                continue
